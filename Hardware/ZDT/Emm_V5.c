...
{
	//清除串口中断标志
    NVIC_ClearPendingIRQ(ZDT1_INST_INT_IRQN);
    // //使能串口中断
    // NVIC_EnableIRQ(ZDT1_INST_INT_IRQN);
	
}

uint8_t ZDT_Switch;
void select_ZDT(uint8_t x)
{
	ZDT_Switch=x;
}

void ZDT_SetSpeed(ZDTx ZDT,int16_t Speed ,uint8_t acc)
{ 
	ZDT_Switch=ZDT;
	if(Speed>=0)
		Emm_V5_Vel_Control(1,0,ABS(Speed),acc,false);
	else 
		Emm_V5_Vel_Control(1,1,ABS(Speed),acc,false);
}
...
{       
  while( DL_UART_isBusy(ZDT1_INST) == true );
  DL_UART_Main_transmitData(ZDT1_INST,data);
